...
export default function CreatorCard(props: CreatorCardProps) {
  ...
  return (
    <div className={`flex flex-col items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm w-44 flex-shrink-0 snap-center hover:border-teal/30 transition-all ${isHistorical ? 'bg-amber-50/30' : ''}`}>
      ...
      <button 
        onClick={handleShare}
        className="absolute top-2 right-2 p-1.5 bg-white/80 backdrop-blur-sm rounded-full text-gray-400 hover:text-teal hover:bg-white transition-colors opacity-0 group-hover:opacity-100"
      >
